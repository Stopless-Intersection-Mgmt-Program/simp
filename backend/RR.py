...
# not finish: doesn't back to origin speed when out
...
class RR(Intersection.Intersection):
    ...
    # def stackArriveAfter(self, car1, car2):
    #     temp = car1.course.pop(-1)
    #     self.arriveAfter(car1, car2)
    #     car1.course.append(temp)
    ...
    def tick(self, period):
                ...
                while i < len(self.cars):
                    if car.path[0] != self.cars[i].path[0]:
                        if rr[self.cars[i].path[0]] <= self.laneCount[car.path[0]]:
                            ...
                            break
                    else: self.arriveAfter(car, self.cars[i])
                    i += 1
        ...
        for i in range(len(self.waiting) - 1, -1, -1):
            if self.waiting[i].distance >= self.startSchedule:
                self.laneCount[self.waiting[i].path[0]] -= 1
                del self.waiting[i]
